...
final class TranslationRenderingSnapshotTests: XCTestCase {
    func testTranslationPanelRendersMarkdownAndMath() throws {
        guard let snapshotPath = ProcessInfo.processInfo.environment["RIFF_SNAPSHOT_PATH"] else {
            throw XCTSkip("Set RIFF_SNAPSHOT_PATH to render the translation panel QA artifact.")
        }
